      ...
      <ul className="hidden md:flex items-center gap-2 text-white/80 font-normal text-sm bg-black/55 backdrop-blur-md border border-white/10 rounded-full p-1.5 shadow-lg">
        {sections.map((sec) => (
          <li
            ...
            className="cursor-pointer hover:text-white transition-colors flex items-center gap-1 px-4 py-1.5 rounded-full relative group"
          >
            ...
            {sec.id === 'decrees' && (
              <ChevronRight className="w-4 h-4 text-white/60 transition-transform group-hover:translate-x-0.5 relative z-10" />
            )}
            {sec.id === 'council' && (
              <ChevronRight className="w-4 h-4 text-white/60 transition-transform group-hover:translate-x-0.5 relative z-10" />
            )}
      ...
      <div className="flex-1 flex justify-end items-center gap-3">
        ...
        >
          ...
          >
            ...
            <span className="text-xs md:text-sm font-normal">Join Us</span>
          </motion.button>
        </a>
      </div>
    </nav>
